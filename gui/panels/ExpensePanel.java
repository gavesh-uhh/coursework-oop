...
public class ExpensePanel extends JPanel{
	...
	public ExpensePanel(Dashboard dashboard, User user) {
		
		this.parentPanel = dashboard;
		
		this.setLayout(null);
		
		JLabel incomeTitleLabel = new JLabel("Expenses...");
		incomeTitleLabel.setFont(new Font("Geist", Font.BOLD, 25));
		incomeTitleLabel.setBounds(25, 25, 621, 48);
		this.add(incomeTitleLabel);
		
		JLabel descriptionLabel = new JLabel("Add Expense Reference");
		...
		valueTextField.getDocument().addDocumentListener(new DocumentListener() {
			...
			public void insertUpdate(DocumentEvent e) {
				try {
					...
					System.err.println("[Error] Cannot Parse String");
				}
			}
		...
		addIncomeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (referenceTextArea.getText().isEmpty()) {
					JOptionPane.showMessageDialog(null, "Write Expense Name", "Something Happened", JOptionPane.ERROR_MESSAGE);
					return;
				}
				user.addCredit(referenceTextArea.getText(), value);
				JOptionPane.showMessageDialog(null, "Expense Added!", "Something Happened!", JOptionPane.ERROR_MESSAGE);
